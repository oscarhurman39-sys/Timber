...
(async () => {
  ...
  check('corrupted storage -> fresh deck, no crash', c.cards === 18 && c.left === '18' && errs.length === 0, JSON.stringify({ c, errs }));
  await ctx.close();
  ...
  await page.goto(URL); await page.waitForTimeout(300);
  for (let i = 0; i < 18; i++) { await page.click('#learn'); await page.waitForTimeout(420); }
  ...
  check('deck cleared before reload', s.empty && s.done === '18', JSON.stringify(s));
  ...
  check('persisted empty deck -> empty state + hidden actions + count kept', s.empty && s.actionsHidden && s.done === '18' && s.cards === 0, JSON.stringify(s));
  ...
  check('reset from restored empty state works', s.cards === 18 && s.done === '0', JSON.stringify(s));
  await ctx.close();
  ...
  check('undo on fresh deck is a safe no-op', c.cards === 18 && c.left === '18' && errs3.length === 0, JSON.stringify({ c, errs3 }));
  await ctx.close();
  ...
  check('undone card returns unflipped with correct counts', flipped === false && c.left === '18' && c.done === '0', JSON.stringify({ flipped, c }));
  await ctx.close();
