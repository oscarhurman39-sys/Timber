...
| Lonicera × purpusii 'Winter Beauty' | lonicera-purpusii-winter-beauty.jpg | centre — Oscar's cat fills roughly half the frame, kept deliberately ("use it as is"). Leaves and stems still clearly identifiable; nursery label visible in-frame confirmed the plant (though the label's plain "Lonicera Winter Beauty" undersells it — corrected on the card to the accepted hybrid name × purpusii) |
...
| Acer shirasawanum MOONRISE | acer-shirasawanum-moonrise-munn-001.jpg | centre — correct autumn red/orange palmate foliage, source is a cutout but the crop fills edge to edge with no visible black void this time |
| Callistemon citrinus 'Splendens' | callistemon-citrinus-splendens.jpg | centre — Oscar composited a second instance of the same bloom into the top-left corner and flagged the seam as not blending well around the red spikes. Touch-up applied: hue/saturation-masked the red bloom region, took its edge band, and feather-blended it against a locally blurred copy to soften any hard cutout line (`tools/last-added-card.png` shows no visible seam at card scale). [Unverified] whether this matches what Oscar saw at full resolution — no distinct hard edge was visually confirmed separate from the photo's natural background bokeh; revisit with a tighter crop from Oscar if the card still shows it |
